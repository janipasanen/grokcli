...
- 2026-03-18: Replaced generic live tool trace lines with action-aware summaries (paths, patterns, patch files, and underlying shell commands such as `swift test` / `cargo test`); approval prompts now include the specific action text.
- 2026-03-18: Blocked shell-based in-place file edits (`sed -i`, `perl -i`, `ruby -i`) so the agent must use `apply_patch`; added verification tracking so the runtime no longer accepts a final success claim while the last build/test verification is still failing; improved streaming chunk-read errors to include the underlying transport detail.
- 2026-03-18: Added live tool result rendering in the terminal: file contents, directory listings, search matches, command stdout/stderr, patch/apply status, and other tool outputs now print immediately after each tool completes.
- 2026-03-18: Replaced plain interactive stdin reading with a readline-style editor in TTY mode, adding Up/Down history navigation and persistent shell history at `~/.local/share/grok-agent/history.txt` while preserving plain stdin fallback for non-interactive runs.
- 2026-03-18: Added interactive task queueing in the shell: Tab and Ctrl+Q now enqueue the current line in TTY mode, queued work is tracked in shell state, `/queue`, `/queue-show`, `/queue-run`, and `/queue-clear` manage the FIFO queue, and queued tasks auto-drain after the next normal prompt completes.
- 2026-03-18: Added `verbose_tools` as a real config/runtime setting, with CLI flags (`--verbose-tools` and `--no-verbose-tools`), interactive `/verbose-tools on|off`, state/header visibility, and runtime gating for live tool traces and tool output previews.
